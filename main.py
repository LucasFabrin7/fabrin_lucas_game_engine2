...
# Design Code: platforms start black and randomly change color when jumped on and the goal is to make them the same color but the red blocks reset the platform color to black and kill you
...
class Player(Sprite):
    ...
    def __init__(self):
        ...
        self.cofric = -0.3
        self.hitpoints = 100
        # sets the key binds for the game
    def controls(self):
...
# set up platforms for the color, and calls the position
...
# code for the mobs that move from side to side down the screen
class Mob(Sprite):
...
# Blue dot that is the objective of the game to hit
class Winning_Dot(Sprite):
...
for winning in WINNING_DOT:
    w = Winning_Dot(*winning)
    all_sprites.add(w)
    all_winning.add(w)
...
# when the player is hit it resets his location
    mhits = pg.sprite.spritecollide(player, all_mobs, True)
    if mhits:
        player.hitpoints -= 1
        player.pos = vec(540, 675)
# Objective of the game and sets up the condition to win
    whits = pg.sprite.spritecollide(player, all_winning, True)
    if whits:
       print("You Win")
